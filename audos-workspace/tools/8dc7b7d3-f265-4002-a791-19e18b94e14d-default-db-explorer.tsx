                      ...
                      {currentColumns.map(col => (
                        <td key={col.name} className="px-3 py-1.5">
                          {isEditing ? (
                            ...
                            >
                              {row[col.name] === null ? <span className="text-gray-400 italic">NULL</span> : String(row[col.name]).length > 50 ? String(row[col.name]).slice(0, 50) + '...' : String(row[col.name])}
                            </span>
                          )}
                        </td>
                      ))}
      ...
      {queryResults && queryResults.rows && (
        <DataTable
          ...
          columns={(queryResults.columns || Object.keys(queryResults.rows[0] || {})).map((col: string) => ({
            ...
            header: col,
            sortable: true,
            render: (val: any) => val === null ? <span className="text-gray-400 italic">NULL</span> : String(val).length > 100 ? String(val).slice(0, 100) + '...' : String(val),
          }))}
          pageSize={25}
          searchable
